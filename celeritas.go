...
import (
	...
	"github.com/luksbutz/celeritas/cache"
	"github.com/luksbutz/celeritas/mailer"
	"github.com/luksbutz/celeritas/render"
	"github.com/luksbutz/celeritas/session"
...
// base on .env files, and creates necessary folders and files if they don't exist
func (c *Celeritas) New(rootPath string) error {
	pathConfig := initPaths{
		rootPath:    rootPath,
		folderNames: []string{"handlers", "migrations", "views", "data", "public", "tmp", "logs", "middleware", "mail"},
	}
	...
	if c.Debug {
		views = jet.NewSet(
			jet.NewOSFileSystemLoader(filepath.Join(rootPath, "views")),
...
// Init creates necessary folders for ou Celeritas application
...
// CheckDotEnv check if a .env file exists
func (c *Celeritas) CheckDotEnv(path string) error {
	err := c.CreateFileIfNotExist(filepath.Join(path, ".env"))
	if err != nil {
		return err
	}
...
// startLoggers starts the info and error logger
...
func (c *Celeritas) createBadgerConn() *badger.DB {
	db, err := badger.Open(badger.DefaultOptions(filepath.Join(c.RootPath, "/tmp/badger")))
	if err != nil {
		return nil
	}
	return db
